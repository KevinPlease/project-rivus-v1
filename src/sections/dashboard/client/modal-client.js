...
const ModalClient = ({formOptions, handleClose, setResetOwner})=>{
    const { user } = useAuth();
    const formik = useFormik({
        ...
        onSubmit: async (values, helpers) => {
            ...
            const responseClientId = await clientAPI.askById(authInfo, undefined);
            const response = await clientAPI.update(authInfo,  responseClientId?.model?._id, values);
