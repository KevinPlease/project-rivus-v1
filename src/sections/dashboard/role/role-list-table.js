...
export const RoleListTable = (props) => {
  ...
  const handledEditClick = (id) => {
    router.push(paths.dashboard.roles.details.replace(":id", id));
  };
        ...
        <Table sx={{ minWidth: 650 }} aria-label="simple table">
          ...
          <TableBody>
            {items.map((item) => (
              ...
              >
                <TableCell align="center">{item.displayId}</TableCell>
                <TableCell align="center">{item.data.name}</TableCell>
                <TableCell align="center">{item.data.actions.length}</TableCell>
